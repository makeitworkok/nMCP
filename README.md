...
![Version](https://img.shields.io/badge/version-0.8.8-orange)
...
| [docs/TOOLS_REFERENCE.md](docs/TOOLS_REFERENCE.md) | All 41 tools with arguments, examples, and response shapes |
| [docs/NIAGARA_OBJECTS_ROADMAP.md](docs/NIAGARA_OBJECTS_ROADMAP.md) | Planned expansions (M1–M5) |
| [docs/LESSONS_LEARNED.md](docs/LESSONS_LEARNED.md) | v0.4.0 through v0.8.8 implementation lessons and operational guidance |
...
Total tools on branch v0.8.8 development line: 41.
...
| v0.8.6 — JSON Parser + Tool Metadata Hardening | ✅ Done | Keeps the zero-dependency JSON helper while adding public top-level array/value parsing, full-document trailing-token rejection, safer number serialization, typed map extraction helpers, and richer agent-facing tool/schema descriptions |
| v0.8.7 — Device Profiling + Runtime Hardening | ✅ Done | Added `nmcp.device.profile`, BQL offset pagination support, invokeAction applied/failed semantics, BACnet runtime compatibility responses, and `equipment.status` network/descendant filtering |
| v0.8.8 — Approved Agent Allowlist Enforcement | ✅ Current | Added BMcpService controls for `requireAgentIdentity`, `approvedAgents`, and `requireApprovedAgent` with fail-closed 400/403 enforcement paths and validated proxy agent-header forwarding |
